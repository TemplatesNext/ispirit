	...
	*/			
	...
	class nx_recent_posts extends WP_Widget {
		...
		function __construct() {	
			//parent::WP_Widget( 'nx_recent_custom_posts', $name = 'NX Recent Posts', array( 'description' => __( 'NX Widget for Recent Posts', 'ispirit' ) ) );			
			
			$widget_ops = array( 'classname' => 'nx_recent_custom_posts', 'description' => 'NX Widget for Recent Posts' );
        	parent::__construct( 'nx_recent_custom_posts', 'NX Recent Posts', $widget_ops );					
		}
			...
			<ul class="recent-posts-list">
				...
				<li class="clearfix nx-recent-post">
					<a href="<?php echo $post_permalink; ?>" class="recent-post-image">
						<?php if ($image) { ?>
						<img src="<?php echo $image[0]; ?>" width="<?php echo $image[1]; ?>" alt=" " />
						<?php } ?>
					</a>
					<div class="recent-post-details <?php echo $thumb_stat; ?>">
						...
						<span><?php printf(__('By %1$s on %2$s', 'ispirit'), $post_author, $post_date); ?></span>
						<div class="comments-likes">
							<?php if ( comments_open() ) { ?>
								<a href="<?php echo $post_permalink; ?>#comment-area"><i class="fa fa-comment"></i><span><?php echo $post_comments; ?></span></a>
							<?php }	?>
						</div>
					</div>
				</li>
		...
		function form( $instance ) {	
				...
				?>
				<p>
					<label for="<?php echo $this->get_field_id('title'); ?>"><?php echo __( 'Title:', 'ispirit' ); ?></label>
					<input id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" class="widefat" />
				</p>
				<p>
					<label for="<?php echo $this->get_field_id('number'); ?>"><?php echo __( 'Number of posts to show:', 'ispirit'); ?></label>
